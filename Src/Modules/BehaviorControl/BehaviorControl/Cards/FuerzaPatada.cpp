...
class FuerzaPatadaCard : public FuerzaPatadaCardBase
{
  ...
  option
  {
    theActivitySkill(BehaviorStatus::FuerzaPatadaCard);
    ...
    state(shortkick)
    {
      ...
      action
      {
        ...
        theKickSkill((KickRequest::kickForwardFastLong), true, 0.1f, false);
      
      }
    }
    state(turnToBallLong)
    {
      transition
      {
        if(!theFieldBall.ballWasSeen(ballNotSeenTimeout))
          goto searchForBallLong;
        if(std::abs(theFieldBall.positionRelative.angle()) < ballAlignThreshold)
          goto walkToBallLong;
      }
